...
public class MediaResourceManager {
	...
	public void getThumbnail(final Media media,
		...
		try {
			BufferedImage bi = ImageIO.read(new File(media.getThumbnailImagePath()));
			observer.thumbnailLoaded(media, bi);
		} catch (IOException e) {
	...
	public void getContentImage(final Media media,
			final MediaResourceObserver observer) {
		try {
			BufferedImage bi = ImageIO.read(new File(media.getContentImagePath()));
			observer.contentImageLoaded(media, bi);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
